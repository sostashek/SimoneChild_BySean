...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    if( $wp_query->current_post == 0 && !is_paged() && is_front_page() ) { // Custom template for the first post on the front page
        if (has_post_thumbnail()) {
            echo '<div class="front-index-thumbnail">';
	...
	<header class="entry-header clear">
		...
		<div class="entry-meta">
            ...
            <?php edit_post_link( sprintf( __( 'Edit', 'simone' ) ), '<span class="edit-link">', '</span>' ); ?>
		</div><!-- .entry-meta -->
		<?php endif; ?>
	</header><!-- .entry-header -->

        <?php
        if( $wp_query->current_post == 0 && !is_paged() && is_front_page() || is_index ) {
